...
document.addEventListener('click', (event) => {
  if (event.target.classList.contains('tabs-layout__item') && event.target.dataset.section === 'our-services') {
  ...
  if (event.target.classList.contains('amazing-work__tabs-layout')) {
    ...
    event.target.classList.add('amazing-work__tabs-layout_add-click-properties');
    filterInAmazingWork(event.target);
  }
})

document.querySelector('.button_adding-items').addEventListener('click', (event) => {
  let clickCount = event.currentTarget.dataset.click;
  clickCount--;
  event.currentTarget.dataset.click = clickCount;
  const currentActiveElement = document.querySelector('.amazing-work__tabs-layout_add-click-properties');
  if (clickCount !== 0) {
    addAllImages();
...
function showTextContent() {
  ...
  const tabsContentCollection = document.querySelectorAll('.content-item__our-services');
...
let imageArray = ['graphic-design1', 'graphic-design2', 'graphic-design3', 'graphic-design4', 'graphic-design5', 'graphic-design6', 'graphic-design7', 'graphic-design8', 'graphic-design9', 'graphic-design10', 'graphic-design11', 'graphic-design12', 'graphic-design13', 'graphic-design14', 'graphic-design15', 'landing-pages1', 'landing-pages2', 'landing-pages3', 'landing-pages4', 'landing-pages5', 'landing-pages6', 'landing-pages7', 'landing-pages8', 'landing-pages9', 'web-design1', 'web-design2', 'web-design3', 'web-design4', 'web-design5', 'web-design6', 'web-design7', 'web-design8', 'web-design9', 'web-design10', 'wordpress1', 'wordpress2', 'wordpress3', 'wordpress4', 'wordpress5', 'wordpress6', 'wordpress7', 'wordpress8', 'wordpress9', 'wordpress10', 'wordpress11', 'wordpress12', 'wordpress13'];

let newImageArray = [];
let count = 0;

function addAllImages() {
  const placeOfAddingCollection = document.querySelectorAll('.portfolio-gallery__card');
  addImagesToNewArray();
  const collectionOfGalleryDescription = document.querySelectorAll('.portfolio-gallery__description');
  for (let index = 0; index < newImageArray.length; index++) {
    placeOfAddingCollection[index].firstElementChild.src = `./img/amazing-works/${newImageArray[index]}.jpg`;
    collectionOfGalleryDescription[index].innerHTML = `${addContentInInner(newImageArray[index])}`;
    placeOfAddingCollection[index].firstElementChild.alt = newImageArray[index];
    placeOfAddingCollection[index].classList.remove('portfolio-gallery__card_add-hidden');
  }
}
...
function resetClassInAmazingWork() {
  const amazingCollection = document.querySelectorAll('.amazing-work__tabs-layout');
  for (let index = 0; index < amazingCollection.length; index++) {
    if (amazingCollection[index].classList.contains('amazing-work__tabs-layout_add-click-properties')) {
      amazingCollection[index].classList.remove('amazing-work__tabs-layout_add-click-properties');
    }
  }
}
...
function filterInAmazingWork(element) {
  ...
  if (element.dataset.filter === 'All') {
    ...
    const collectionOfGalleryDescription = document.querySelectorAll('.portfolio-gallery__description');
    for (let index = 0; index < collectionOfGalleryDescription.length; index++) {
      if (collectionOfGalleryDescription[index].innerHTML !== filter && collectionOfGalleryDescription[index].innerHTML !== '') {
        currentElements[index].classList.add('portfolio-gallery__card_additional-hidden');
      }
    }
  }
}
...
function removeClassHiddenInFilter() {
  const currentElements = document.querySelectorAll('.portfolio-gallery__card_additional-hidden');
  for (let index = 0; index < currentElements.length; index++) {
    currentElements[index].classList.remove('portfolio-gallery__card_additional-hidden');
  }
}
...
function createMasonryElement(counter) {
  ...
  if (counter % 3 == 0) {
    wrapperOfElement.classList.add('masonry-item', 'masonry-item--width4', 'masonary-item-properties');
  } else {
    wrapperOfElement.classList.add('masonry-item', 'masonry-item--width3', 'masonary-item-properties');
  }  
...
function imageForMasonry(elementNumber) {
  ...
  imageForAdding.src = `./img/best-images/best-images${elementNumber}.png`;
...
function createLinkInMasonryElement(param) {
  ...
  if (param === 'search') {
    linkSearch.classList.add('wrapper-for-gallery__search', 'wrapper-for-gallery__size', 'wrapper-for-gallery__background-color');
    linkSearch.href = '#';
  } else {
    linkSearch.classList.add('wrapper-for-gallery__zoom', 'wrapper-for-gallery__size', 'wrapper-for-gallery__background-color');
    linkSearch.href = '#';
  }
